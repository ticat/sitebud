...
export function SearchInput(props: SearchInputProps) {
    ...
    const handleInputChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (typeof window !== 'undefined') {
            if (value && value.length >= 3) {
                window.localStorage.setItem('searchPage_searchText', value);
            } else {
                window.localStorage.removeItem('searchPage_searchText');
            }
        }
        setSearchText(value);
    };
    ...
    useEffect(() => {
        const storedSearchText: string | null = window.localStorage.getItem('searchPage_searchText');
        if (storedSearchText) {
    ...
    return (
        <div className="relative flex items-center mt-4 md:mt-0 w-full">
